...
public class MarathonSpec extends BaseGSpec {
    ...
    public void checkNumberOfTasksHealthinessWithPolling(int timeout, int pause, String appId, int numberOfTasks, String state) throws Exception {
        ...
        while (time < timeout) {
            ...
            switch (state) {
                case "healthy":   count = app.getApp().getTasksHealthy();
                                  break;
                case "unhealthy": count = app.getApp().getTasksUnhealthy();
                                  break;
                case "staged":    count = app.getApp().getTasksStaged();
                                  break;
                case "running":   count = app.getApp().getTasksRunning();
                                  break;
                default:          count = 0;
            }
